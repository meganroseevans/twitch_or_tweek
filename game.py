...
class BirdGame:
    ...
    def get_multichoice_options(self, target_bird_category):
        ...
        full_bird_list = [re.sub('\s\(.+\)','',x) for x in self.bird_image_df.name]
        short_bird_list = [re.sub('\s\(.+\)','',x) for x in self.bird_image_df.name[self.bird_image_df.category == target_bird_category]]
